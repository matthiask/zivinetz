...
from pdfdocument.document import PDFDocument, PageTemplate, Frame, cm, mm
...
def generate_expense_statistics_pdf(reports):
    ...
    pdf.doc.addPageTemplates([
        PageTemplate(id='First', frames=[
            Frame(.5*cm, .5*cm, 19.8*cm, 28.5*cm, showBoundary=0, leftPadding=0,
    ...
    for report in reports:
        ...
        working_day = add('spending_money accomodation_%(t)s breakfast_%(t)s lunch_%(t)s supper_%(t)s' % {'t': 'working'})
        free_day = add('spending_money accomodation_%(t)s breakfast_%(t)s lunch_%(t)s supper_%(t)s' % {'t': 'free'})
        sick_day = add('spending_money accomodation_%(t)s breakfast_%(t)s lunch_%(t)s supper_%(t)s' % {'t': 'sick'})
    ...
    def _add_sum(reports, title=''):
        ...
        pdf.table([
            ['Total %s' % title, '', ''] + [sum(transposed[i], 0) for i in range(3, 18)],
