...
(function($) {
    ...
    $(document.body).on('click', 'input.batch:checkbox', function() {
        var cb = $(this);
        if (cb.prop('value') == 'on') {
            cb.closest('table').find('tbody input.batch:checkbox').prop(
                'checked', cb.prop('checked'));
        } else {
            cb.closest('table').find('thead input.batch:checkbox').prop(
                'checked', false);
        }
    });
...
(function($) {
    $(document.body).on('editLive', function(event, elem) {
        if (elem.is(':checkbox'))  /* TODO NOT text input. selects! */
            elem = elem.parent();
