...
class SpecificationForm(forms.ModelForm):
    class Meta:
        ...
        for type in ("accomodation", "breakfast", "lunch", "supper"):
            fields.extend(["%s_working" % type, "%s_sick" % type, "%s_free" % type])


class DrudgeSearchForm(SearchForm):
    orderings = {"date_joined": "user__date_joined"}
...
class AssignmentSearchForm(SearchForm):
    specification__scope_statement = forms.ModelMultipleChoiceField(
        ScopeStatement.objects.all(), label=_("scope statements"), required=False
    )
    ...
    service_between = forms.DateField(
        ...
        required=False,
        widget=forms.DateInput(attrs={"class": "dateinput"}),
        help_text=_("Drudges in service any time between the following two dates."),
    )
...
class AbsenceSearchForm(SearchForm):
    def queryset(self, model):
        return (
            super()
            .queryset(model)
            .select_related("assignment__drudge__user", "created_by")
        )


class ExpenseReportSearchForm(SearchForm):
    default = {"assignment__status": (Assignment.ARRANGED, Assignment.MOBILIZED)}
    ...
    assignment__status = forms.MultipleChoiceField(
        choices=Assignment.STATUS_CHOICES, label=_("assignment status"), required=False
    )
    status = forms.MultipleChoiceField(
        choices=ExpenseReport.STATUS_CHOICES, label=_("status"), required=False
    )
    ...
    def queryset(self, model):
        return (
            super()
            .queryset(model)
            .select_related("assignment__specification", "assignment__drudge__user")
        )
...
class AssignDrudgesToGroupsForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        if not assignments:
            ...
            assignments = dict(
                GroupAssignment.objects.for_date(
                    self.day - timedelta(days=7)
                ).values_list("assignment", "group")
            )
        ...
        for asg in self.assignments.select_related(
            ...
            self.fields["asg_%s" % asg.id] = f = forms.ModelChoiceField(
                ...
                initial=(
                    assignments.get(asg.id)
                    or asg.specification.scope_statement.default_group_id
                ),
        ...
        for asg in self.assignments:
            ...
            if group:
                GroupAssignment.objects.update_or_create(
                    assignment=asg, week=self.day, defaults={"group": group}
                )
            else:
                GroupAssignment.objects.filter(assignment=asg, week=self.day).delete()
