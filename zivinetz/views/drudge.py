    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("regional_office"):
            queryset = queryset.filter(
                regional_office=self.request.GET.get("regional_office")
            )
        if self.request.GET.get("specification"):
            queryset = queryset.filter(
                specification=self.request.GET.get("specification")
            )
...
class DrudgePDFExportView(BaseView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get("environment_course") is not None:
            queryset = queryset.filter(
                environment_course=self.request.GET.get("environment_course")
            )
        if self.request.GET.get("motor_saw_course") is not None:
            queryset = queryset.filter(
                motor_saw_course=self.request.GET.get("motor_saw_course")
            )
