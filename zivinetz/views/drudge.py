...
class AssignmentExportBaseView(BaseView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get('regional_office'):
            queryset = queryset.filter(regional_office=self.request.GET.get('regional_office'))
        if self.request.GET.get('specification'):
            queryset = queryset.filter(specification=self.request.GET.get('specification'))
...
class AssignmentPDFExportView(AssignmentExportBaseView):
    ...
    def add_assignment_data(self, pdf, assignment):
        ...
        pdf.table([
            ...
            (_("Name"), f"{assignment.drudge.user.last_name}, {assignment.drudge.user.first_name}"),
            (_("Email"), assignment.drudge.user.email),
            (_("Phone"), f"{assignment.drudge.phone_home or '-'} / {assignment.drudge.mobile or '-'}"),
            (_("Kurse"), ", ".join(courses) if courses else "-"),
            (_("Pflichtenheft"), assignment.specification.code),
            (_("Date"), f"{self.format_date(assignment.date_from)} - {self.format_date(assignment.determine_date_until())}"),
...
class DrudgePDFExportView(BaseView):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get('regional_office'):
            queryset = queryset.filter(regional_office=self.request.GET.get('regional_office'))
        if self.request.GET.get('environment_course') is not None:
            queryset = queryset.filter(environment_course=self.request.GET.get('environment_course'))
        if self.request.GET.get('motor_saw_course') is not None:
            queryset = queryset.filter(motor_saw_course=self.request.GET.get('motor_saw_course'))
    ...
    def get_active_status(self, drudge):
        ...
        if active_assignments.exists():
            return "Aufgeboten"
        return drudge.get_status_display() if hasattr(drudge, 'get_status_display') else '-'
    ...
    def get(self, request, *args, **kwargs):
        ...
        queryset = self.get_queryset().select_related(
            ...
            'assignments',
            'assignments__specification',
            'assignments__assessments'
        )
        ...
        if request.GET.get('s'):
            ...
            for key, value in request.GET.items():
                if key != 's' and value:
                    param_text = self.format_search_parameter(key, value)
                    y = self.draw_wrapped_text(p, param_text, margin + cm, y, width - (2 * margin), font_name, 10)
            y -= line_height
        ...
        for i in range(0, len(queryset), batch_size):
            ...
            for drudge in batch:
                ...
                assignments_list = []
                for assignment in drudge.assignments.all():
                    date_from = assignment.date_from.strftime('%d.%m.%Y') if assignment.date_from else '-'
                    date_until = assignment.determine_date_until().strftime('%d.%m.%Y') if assignment.determine_date_until() else '-'
                    assignments_list.append(f"{assignment.specification.code} ({date_from} - {date_until})")
                ...
                y = self.draw_wrapped_text(p, address_text, x, y, width - margin - x, font_name, 9)
                ...
                p.setFont(font_name, 9)
                y = self.draw_wrapped_text(p, drudge.user.email, x, y, width - margin - x, font_name, 9)
                ...
                y = self.draw_wrapped_text(p, education_text, x, y, width - margin - x, font_name, 9)
                ...
                p.setFont(font_name, 9)
                y = self.draw_wrapped_text(p, last_assignment_str, x, y, width - margin - x, font_name, 9)
                ...
                p.setFont(font_name, 9)
                y = self.draw_wrapped_text(p, assignments_str, x, y, width - margin - x, font_name, 9)
